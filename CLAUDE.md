...
Both repositories live in the **`explain-labs`** GitHub organization: this app is [`explain-labs/explain-ui`](https://github.com/explain-labs/explain-ui) and the engine submodule is [`explain-labs/explain-engine`](https://github.com/explain-labs/explain-engine) (recorded in `.gitmodules` as `git@github.com:explain-labs/explain-engine.git`). Both moved there from the personal `Dobutamine` account; GitHub still redirects the old URLs, so a pre-move clone keeps working, but anything you write should use the org URLs. Shared cross-repo usage instructions live on the [org profile](https://github.com/explain-labs), sourced from `explain-labs/.github`. A checkout made before the move needs `git submodule sync` to pick up the new submodule URL.
